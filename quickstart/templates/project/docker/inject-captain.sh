...
app_root="${1:?missing app root}"
app_name="${QUICKSTART_APP_NAME:?missing QUICKSTART_APP_NAME}"
app_module="${QUICKSTART_APP_MODULE:?missing QUICKSTART_APP_MODULE}"
web_module="${app_module}Web"
context_dir="$app_root/lib/$app_name"
web_dir="$app_root/lib/${app_name}_web"
controller_dir="$web_dir/controllers"
router_file="$web_dir/router.ex"

mkdir -p "$context_dir" "$controller_dir"

cat > "$context_dir/captain.ex" <<EOF
defmodule ${app_module}.Captain do
  @moduledoc false
...
cat > "$controller_dir/captain_controller.ex" <<EOF
defmodule ${web_module}.CaptainController do
  use ${web_module}, :controller

  alias ${app_module}.Captain
            ...
            <form action="/captain" method="post">
              ...
              <div class="composer-footer">
                <div class="composer-note">
                  Keep requests small and concrete. Captain tasks can queue up freely, but Reverb only works one change at a time.
                </div>
...
if ! grep -q 'get "/captain", CaptainController, :index' "$router_file"; then
  tmp_router="$(mktemp)"

  awk '
    /get "\/", PageController, :home/ && !done {
      print
      print "    get \"/captain\", CaptainController, :index"
      print "    post \"/captain\", CaptainController, :create"
      done = 1
      next
    }
    { print }
