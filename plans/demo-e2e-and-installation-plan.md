...
Suggested example app shape:

- a fresh Phoenix app generated by `mix phx.new`
...
- use the stock Phoenix layout generated by `mix phx.new`
- inject Reverb configuration with the installer path
- layer on a small `/captain` surface for no-code steering
...
- optional future human-preview container if the quickstart needs one later
...
- isolated mutable checkout of the generated app
...
- boot the generated Phoenix app in emitter mode
- expose the user-facing UI on `localhost:4000`
- host the `/captain` steering surface

`reverb`:

- boot in receiver mode
- connect to the generated prod node
- maintain durable task/run state
- manage workspaces against the generated app checkout
- run validation commands inside that checkout
...
For the generated quickstart app, use narrow commands only:
...
Add enough visibility that the quickstart stack is inspectable without digging into
...
This does not need to block the quickstart smoke path if shell inspection is enough.
...
Create a quickstart walkthrough that verifies:
...
- waits for the generated Phoenix app and OpenCode web to come up
- inspects Reverb status
- confirms a captain request is implemented in the isolated checkout
...
After the quickstart app succeeds, use Reverb as a target for Reverb itself.
...
1. complete quickstart app loop
...
### Phase A: Quickstart App

- scaffold a fresh Phoenix app with `mix phx.new`
- install Reverb into it using the generator path
- add `/captain` steering and default logger-based emission
- keep validation narrow and fast

### Phase B: Compose Environment

- add a quickstart compose topology around the generated app
- add the coordinator container and OpenCode web surface
- persist app DB, Reverb DB, workspaces, and OpenCode auth
